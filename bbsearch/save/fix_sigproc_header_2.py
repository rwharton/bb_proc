...
def fix_header(filename, keyword, new_value):
    ...
    print(filename, keyword, new_value)
    ...
    if isinstance(value_dtype, bytes):
        ...
        if (keyword == "source_name"):
            val_str = to_sigproc_keyword(keyword, str(new_value))
...
def to_sigproc_keyword(keyword, value=None):
    ...
    if value_dtype is str:
        return np.int32(len(keyword)).tobytes() + keyword.encode('ascii') + np.int32(len(value)).tobytes() + value.encode("ascii")
...
@click.command()
@click.option("--filterbank", help="Filterbank filename.", type=str)
...
def run_fix_header(filterbank, telescope, dsn):
     
    if (telescope == "RO"):
        telescope_id = 14
        keyword = "telescope_id"
        fix_header(filterbank, keyword, telescope_id)
    ...
    if dsn:
        machine_id = 999
        keyword = "machine_id"
        fix_header(filterbank, keyword, machine_id)
