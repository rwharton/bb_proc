...
class SP_CAND:
    def __init__(self, line, ctype, num):
        self.dm = None
        self.snr = None
        self.time = None
        self.samp = None
...
def t_dm_shifts2(dDMs, nchan, fch1, df):
    """
    calc time offset from incorrect dm
    """
    fchans = np.arange(nchan) * df + fch1
    ...
    dt_sweeps = 4.15e3 * avg_f2 * dDMs
    #dt_sweeps = 4.15e3 * (flo**-2.0 - fhi**-2.0) * dDMs

    dt_infs = 4.15e3 * fhi**-2. * dDMs

    dts = dt_sweeps - dt_infs
...
def sift_dm_dupes(clist, nchan, fch1, df, dt):
    ...
    while(len(c_snrs)):
        ...
        dts = t_dm_shifts2(dms-dm0, nchan, fch1, df)
...
def cross_check_clists(clist1, clist2, nchans, fch1, df, dt):
    ...
    for ii in range(len(clist1)):
        ...
        dm_dt = t_dm_shifts2(dm1[ii]-dm2[jj], nchans, fch1, df)
...
def get_approx_dfac(num_in, num_out, num_min=8):
    """
    Get the closest downfactor term to get 
    num_out from num_in
    """
    ifac = int( num_in / num_out )
    fac = max(ifac, num_min)
    return fac
...
def get_csift(clist, data_file):
    ...
    """
    Make h5 files from snippets
    Only save candidates with snr > snr_min

    t_dec == - 1 means use width//2

    tcand == -1 means center of snippet
    """
    # Get file list
    filfiles = glob.glob('%s/*fil' %indir)
    fnames = [fn.split('/')[-1] for fn in filfiles]
    fbases = [fn.split('.fil')[0] for fn in fnames ]
    
    # now convert to your format and save
    Nc = len(filfiles)
    for ii, infile in enumerate(filfiles):
        print("Cand %d/%d" %(ii, Nc))
    
        # convert to your format 
        yc = Candidate(fp=infile, dm=dm)
        
        # Get tcand if nec
        if tcand < 0:
            dt = yc.your_header.tsamp
            nspec = yc.your_header.nspectra
            tcand = 0.5 * dt * nspec
            yc.tcand = tcand
        ...
        # Fix annoying ra/dec null h5 issue by setting 
        # to 0.0 if not found.  Note: this is only ok 
        # if you are dealing with topocentric data
        if ( (yc.ra_deg is None) and (yc.dec_deg is None) ):
            print("RA and Dec unset in header!")
            print("Defaulting to 0.0 / 0.0")
            yc.ra_deg = 0.0
            yc.dec_deg = 0.0
            yc.your_header.ra_deg = 0.0
            yc.your_header.dec_deg = 0.0
            yc.your_header.gl = 0.0
            yc.your_header.gb = 0.0
        ...
        # Remove bandpass
        Nthird = int( yc.dedispersed.shape[0] // 3)
        spec = np.mean(yc.dedispersed[0:Nthird], axis=0)
        yc.dedispersed = yc.dedispersed / spec - 1

        # Save data 
        outfile = "%s.h5" %(fbases[ii])
        fout = yc.save_h5(fnout=outfile)
        #outfile = "%s.npy" %(fbases[ii])
        #np.save(outfile, yc.dedispersed)
    return 
...
def make_h5_from_snippets_sp(sp, fildir, basename, tcand=-1, snr_min=7, wmax=-1, 
                             zap_chans=None, t_dec=-1, f_dec=1, min_samp=0, 
                             n=-1, ntmax=-1, extra_filter=False):
    """
    Make h5 candidates for snippets from spfile
    Only save candidates with snr > snr_min
    ...
    if type(sp) == str:
        clist_all = cands_from_spfile(sp, 'sp')
    ...
    for ii, cc in enumerate(clist):
        ...
        # convert to your format 
        yc = Candidate(fp=infile, dm=cc.dm, snr=cc.snr)

        # Make sure file isnt empty
        if yc.your_header.nspectra == 0:
            print("Empty file: %s" %(infile))
            print("Skipping")
            continue
        else: pass
        ...
        fname = infile.rsplit('/')[-1]
        fn_base = fname.rsplit('.fil')[0]
        outfile = "%s.h5" %fn_base
...
def sum_pols_one(basename):
    """
    read in [basename]_LCP.npy and 
            [basename]_RCP.npy 
    make    [basename]_I.npy
    """
    Lfile = "%s_LCP.npy" %basename
    Rfile = "%s_RCP.npy" %basename
    Ifile = "%s_I.npy" %basename
    ...
    Nt = datL.shape[0]
    N3 = int( Nt // 3)
    xx = np.hstack( (np.arange(0, N3), np.arange(Nt-N3, Nt)) )
...
def sum_pols_many(indir):
    """
    combine npy files
    """
    glob_str = "%s/*CP.npy" %indir
    print(glob_str)
    npy_files = glob.glob(glob_str)
    fbases = [ fn.rsplit("_", 1)[0] for fn in npy_files ]
    fbases = list( set(fbases) )
...
def make_h5_plots():
    """
    try / except to catch bad data
    """
    h5_files = glob.glob("*h5")
    for hfile in h5_files:
        try:
            call("your_h5plotter.py -f %s" %hfile, shell=True)
        except:
            print("Problem with %s" %hfile)
    return
...
def parse_input():
    ...
    return infile, fildir, bname, tdec, fdec, nmin, nmax, snr, wmax
...
if __name__ == "__main__":
    if debug:
        ...
        infile, fildir, bname, tdec, fdec, nmin, nmax, snr, wmax = parse_input()

        # Generate *h5 data snippets from cand file
        # and make plots from *h5 files
        make_h5_from_snippets_sp(infile, fildir, bname, snr_min=snr, wmax=wmax, 
                                 tcand=-1, zap_chans=None, t_dec=tdec, f_dec=fdec, 
                                 min_samp=0, n=nmin, ntmax=nmax)
